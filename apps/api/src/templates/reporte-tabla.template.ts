...
export interface DatosTablaSimple {
  ...
  /** Opcional: fuerza que todas las columnas tengan el mismo ancho (tabla de tipo horario/grilla). */
  anchoIgual?: boolean
}
...
export function generarHTMLTablaSimple(d: DatosTablaSimple): string {
  const anchoCol = d.anchoIgual ? `width:${(100 / d.columnas.length).toFixed(4)}%;` : ''

  const headerCols = d.columnas.map(c =>
    `<th style="background:${AZUL};color:#fff;padding:5px 6px;font-size:9px;text-align:${c.align ?? 'left'};${anchoCol}">${c.header}</th>`
  ).join('')

  const filas = d.filas.map((fila, idx) => {
    const bg = idx % 2 === 0 ? '#FFFFFF' : '#F5F5F5'
    const celdas = d.columnas.map(c =>
      `<td style="padding:4px 6px;font-size:9px;text-align:${c.align ?? 'left'};${anchoCol}">${fila[c.key] ?? '—'}</td>`
    ).join('')
...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  table { border-collapse: collapse; width: 100%; ${d.anchoIgual ? 'table-layout: fixed;' : ''} }
  th, td { border: 1px solid #ddd; ${d.anchoIgual ? 'word-wrap: break-word; overflow-wrap: break-word;' : ''} }
